    ...
    return (<div className="flex flex-col h-full bg-[#f8f9fa] text-black font-sans">
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-white border-b border-gray-300">
        <div className="flex items-center gap-3">
          <Menu size={20}/>
          {editingTitle ? (<input className="text-lg font-bold text-gray-800 border-b border-blue-400 bg-transparent outline-none px-1 w-64" value={notebookTitle} onChange={e => setNotebookTitle(e.target.value)} onBlur={() => setEditingTitle(false)} onKeyDown={e => { if (e.key === 'Enter')
            setEditingTitle(false); }} autoFocus/>) : (<div className="flex items-center gap-2">
      ...
      <div className="flex-1 overflow-y-auto bg-white">
          <div className="max-w-4xl mx-auto p-4">
              {cells.map((cell, idx) => (<ColabCell key={cell.id} ref={cellRefs.current[idx]} cell={cell} onChange={updateCell} onDelete={deleteCell} onMoveUp={() => moveCell(cell.id, 'up')} onMoveDown={() => moveCell(cell.id, 'down')} onRun={runCell} onClearOutput={clearCellOutput}/>))}
          </div>
      </div>
    </div>);
}
