    ...
    return (<div className="h-full flex flex-col">
      ...
      <div className="bg-[#1e1e1e] text-white p-4 border-b border-gray-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={handleFullScreen} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition flex items-center gap-2" title="Toggle Full Screen">
            {isFullScreen ? "⛶ Exit Full Screen" : "⛶ Full Screen"}
          </button>
        </div>
        
        <div className="flex flex-wrap gap-2 mb-4">
          {vscodeOptions.map((option) => (<button key={option.id} onClick={() => {
                setSelectedVSCode(option.id);
                setIsLoading(true);
                setError(null);
            }} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${selectedVSCode === option.id
        ...
        {currentVSCode && (<p className="text-sm text-gray-400">
      ...
      <div ref={containerRef} className="flex-1 relative bg-[#1e1e1e]" style={{
        ...
        {isLoading && (<div className="absolute inset-0 bg-[#1e1e1e] flex items-center justify-center z-10">
        ...
        {error && (<div className="absolute inset-0 bg-[#1e1e1e] flex items-center justify-center z-10">
        ...
        {currentVSCode && (<iframe src={currentVSCode.url} style={{
                width: "100%",
                height: "100%",
                border: "none",
                background: "#1e1e1e"
            }} title={`${currentVSCode.name} - VS Code Web`} allowFullScreen allow="accelerometer; camera; encrypted-media; geolocation; gyroscope; microphone; midi; clipboard-read; clipboard-write" onLoad={handleIframeLoad} onError={handleIframeError}/>)}
      </div>
      ...
      <div className="bg-[#007acc] text-white px-4 py-2 flex items-center justify-between text-sm">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => window.open(currentVSCode?.url, '_blank')} className="hover:bg-blue-700 px-2 py-1 rounded transition">
            🔗 Open in New Tab
          </button>
          <button onClick={() => {
