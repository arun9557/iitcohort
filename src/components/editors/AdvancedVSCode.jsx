...
export default function AdvancedVSCode() {
    ...
    const currentConfig = vscodeConfigs[selectedTab];
    useEffect(() => {
        setIsLoading(true);
        setError(null);
    ...
    return (<div className="h-full min-h-0 flex flex-col flex-1 bg-[#181a20] font-sans" style={{ height: '100%', minHeight: 0 }}>
      {/* Provider Bar */}
      <div className="flex items-center justify-between px-0 py-0 bg-[#181a20]" style={{ minHeight: 0 }}>
        {/* Providers */}
        <div className="flex flex-1">
          {Object.entries(vscodeConfigs).map(([key, config]) => (<button key={key} onClick={() => setSelectedTab(key)} className={`px-4 py-3 text-sm font-medium transition whitespace-nowrap border-0 rounded-none focus:outline-none focus:bg-[#1e293b] ${selectedTab === key
                ? "bg-[#2563eb] text-white"
                : "bg-transparent text-gray-300 hover:bg-[#23272e] hover:text-white"}`} style={{ boxShadow: 'none', borderRadius: 0, margin: 0, fontWeight: 500, letterSpacing: 0.1 }}>
              {config.name}
            </button>))}
        </div>
        {/* Minimal Buttons */}
        <div className="flex items-center gap-2 pr-4">
          <button onClick={openInNewTab} className="px-3 py-1 text-xs font-medium bg-[#2563eb] text-white rounded hover:bg-[#1d4ed8] transition border-0 shadow-none">
            Open in New Tab
          </button>
          <button onClick={copyUrl} className="px-3 py-1 text-xs font-medium bg-[#23272e] text-gray-200 rounded hover:bg-[#1e293b] transition border-0 shadow-none">
            Copy URL
          </button>
          <button onClick={handleFullScreen} className="px-3 py-1 text-xs font-medium bg-[#23272e] text-gray-200 rounded hover:bg-[#1e293b] transition border-0 shadow-none">
            {isFullScreen ? "Exit Full Screen" : "Full Screen"}
          </button>
        </div>
      </div>
      {/* Description */}
      {currentConfig && (<div className="px-4 py-2 bg-[#23272e] text-gray-200 text-sm font-normal border-0" style={{ letterSpacing: 0.05 }}>
          <span className="font-semibold text-white">{currentConfig.name}:</span> {currentConfig.description}
          <div className="flex flex-wrap gap-2 mt-2">
            {currentConfig.features.map((feature, index) => (<span key={index} className="bg-[#2563eb] text-white px-2 py-1 rounded text-xs font-medium">
                {feature}
              </span>))}
          </div>
        </div>)}
      {/* VS Code Container */}
      <div ref={containerRef} className="flex-1 min-h-0 relative bg-[#181a20]" style={{
            borderRadius: 0,
            overflow: "hidden",
            border: "none",
            height: '100%',
        ...
        {isLoading && (<div className="absolute inset-0 bg-[#181a20] flex items-center justify-center z-10">
        ...
        {error && (<div className="absolute inset-0 flex items-center justify-center bg-[#181a20] z-10">
            <div className="bg-[#23272e] text-red-400 px-6 py-4 rounded text-sm shadow-none border-0">
              {error}
            </div>
          </div>)}
        <iframe src={currentConfig?.url} title={currentConfig?.name} className="w-full h-full min-h-0 min-w-0 border-0" style={{ minHeight: 0, minWidth: 0, background: '#181a20', height: '100%' }} onLoad={handleIframeLoad} onError={handleIframeError} allowFullScreen/>
      </div>
    </div>);
}
