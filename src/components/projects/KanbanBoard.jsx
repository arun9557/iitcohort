...
import { SortableContext, sortableKeyboardCoordinates, verticalListSortingStrategy, useSortable, rectSortingStrategy, } from '@dnd-kit/sortable';
...
function SortableTask({ task, onClick, columnId }) {
    ...
    return (<div ref={setNodeRef} style={style} className={`bg-white rounded-2xl p-5 mb-4 cursor-pointer transition-all duration-200 select-none ${isDragging ? 'opacity-90 scale-105 ring-2 ring-blue-500' : 'hover:shadow-lg'} group border border-gray-100`} onClick={() => onClick(task)}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="cursor-grab p-1 rounded hover:bg-blue-50 active:bg-blue-100 border border-gray-200 group-hover:border-blue-400 transition" {...attributes} {...listeners} onClick={e => e.stopPropagation()}>
      ...
      <div className="flex items-center gap-2 mb-2 text-xs text-gray-800">
        ...
        {isOwner(task.assignee) && (<span className="ml-2 px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold border border-yellow-200">ADMIN</span>)}
...
function KanbanColumn({ column, children }) {
    ...
    return (<motion.div ref={setNodeRef} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className={`bg-white rounded-3xl p-5 shadow-lg min-h-[350px] transition-all duration-200 border border-gray-100 ${isOver ? 'ring-2 ring-blue-400' : ''}`}>
      {children}
    </motion.div>);
}
// Add Modal component
function ProjectModal({ open, onClose, project, onSave }) {
    ...
    if (!open || !edit)
        return null;
    return (<div className="fixed inset-0 z-50 w-full h-full flex items-start justify-center bg-transparent">
          ...
          <div className="space-y-7">
            ...
            <div className="flex gap-6">
              <div className="flex-1">
                ...
                <select className="w-full bg-gray-100 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-blue-400 outline-none" value={edit.status} onChange={e => setEdit({ ...edit, status: e.target.value })}>
...
export default function KanbanBoard({ projectId }) {
    ...
    return (<div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-lg shadow-sm">
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd} onDragStart={({ active }) => {
            const allTasks = columns.flatMap(col => col.tasks);
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {columns.map((column, idx) => (<KanbanColumn key={column.id} column={column}>
                  ...
                  <SortableContext items={column.tasks.map(task => task.id)} strategy={verticalListSortingStrategy}>
                    {column.tasks.map((task) => (<SortableTask key={task.id} task={task} onClick={handleCardClick} columnId={column.id}/>))}
                  </SortableContext>
                  <button onClick={() => addTask(column.id)} className="w-full mt-4 p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl transition flex items-center justify-center gap-2 font-semibold shadow-sm border border-blue-100">
        ...
        <DragOverlay>
          {activeTask && (<div className="bg-white rounded-2xl p-5 mb-4 cursor-pointer transition-all duration-200 select-none opacity-95 scale-105 ring-2 ring-blue-500 shadow-2xl border border-blue-200">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 rounded-full bg-blue-500"></span>
              ...
              <div className="flex items-center gap-2 mb-2 text-xs text-gray-800">
                ...
                {isOwner(activeTask.assignee) && (<span className="ml-2 px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-full text-xs font-semibold border border-yellow-200">ADMIN</span>)}
                <Calendar className="w-3 h-3 text-gray-400 ml-3"/>
