...
export default function MemberList({ currentUserId }) {
    ...
    useEffect(() => {
        ...
        const handleUserConnected = (otherUserId) => {
            if (!stream || !otherUserId || otherUserId === currentUserId)
                return;
        ...
        const handleIncomingSignal = (payload) => {
            ...
            peer.on('stream', (remoteStream) => {
                ...
                let audioElement = document.getElementById(`user-audio-${callerID}`);
                if (!audioElement) {
    ...
    const createPeer = (userToSignal, callerID, stream) => {
        ...
        peer.on('stream', (remoteStream) => {
            ...
            let audioElement = document.getElementById(`user-audio-${userToSignal}`);
            if (!audioElement) {
                audioElement = document.createElement('audio');
    ...
    const setupAudioMeter = (audioStream, userId) => {
    ...
    return (<div className="space-y-4 p-4">
      ...
      <AudioRoom roomId="default-room" // You might want to make this dynamic based on your app's routing
     userId={currentUserId} onStreamReady={handleStreamReady} onStreamError={handleError}/>
      
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">Members ({members.length})</h2>
        <button onClick={toggleMute} className={`p-2 rounded-full ${isMuted ? 'bg-red-500' : 'bg-green-500'} text-white`} title={isMuted ? 'Unmute' : 'Mute'}>
          {isMuted ? <FaMicrophoneSlash /> : <FaMicrophone />}
        </button>
      </div>
      
      <div className="space-y-2">
        {members.map((member) => (<div key={member.uid} className={`flex items-center p-3 rounded-lg ${member.uid === currentUserId ? 'bg-blue-50' : 'bg-white'} ${speaking[member.uid] ? 'ring-2 ring-red-400' : ''}`}>
            <div className="mr-3">
              {member.role === 'teacher' ? (<FaUserTie className="text-blue-600"/>) : member.role === 'student' ? (<FaGraduationCap className="text-green-600"/>) : (<FaUser className="text-gray-500"/>)}
            </div>
            ...
            <div className="flex items-center ml-2">
              <div className={`w-3 h-3 rounded-full mr-2 ${muted[member.uid] ? 'bg-red-500' : 'bg-green-500'}`} title={muted[member.uid] ? 'Muted' : 'Unmuted'}/>
              {speaking[member.uid] && (<div className="w-3 h-3 rounded-full bg-red-500 animate-pulse"/>)}
            </div>
          </div>))}
      </div>
    </div>);
