...
export default function Sidebar({ onSelect, activeTab }) {
    ...
    return (<aside className="w-64 min-h-screen bg-glass glass-card shadow-2xl border border-white/30 flex flex-col backdrop-blur-xl transition-all duration-300">
      {/* Header */}
        ...
        {navSections.map(section => (<div key={section.title}>
            ...
            <ul className="space-y-1">
              {section.items.map(item => (<li key={item.key}>
                  <button onClick={() => onSelect(item.key)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium transition-all duration-200 group relative overflow-hidden
                      ${activeTab === item.key
                    ...
                    {activeTab === item.key && (<span className="absolute right-2 top-1/2 -translate-y-1/2 w-2 h-2 bg-blue-400 rounded-full shadow-lg animate-pulse"/>)}
                  </button>
                </li>))}
            </ul>
          </div>))}
