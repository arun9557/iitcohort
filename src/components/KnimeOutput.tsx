      ...
      <aside className="w-64 bg-white border-r p-6 flex flex-col justify-between">
        <div>
          ...
          <ul className="text-blue-700 text-sm space-y-1 mb-4">
            {resources.map(r => (
              <li key={r.label}><a href={r.url} className="hover:underline">{r.label}</a></li>
            ))}
          </ul>
          ...
          <div className="flex items-center overflow-x-auto pb-8 pt-4">
            {nodes.map((node, idx) => (
              <React.Fragment key={node.key}>
                <div
                  className={`flex flex-col items-center cursor-pointer group ${selected === node.key ? 'ring-2 ring-blue-400' : ''}`}
